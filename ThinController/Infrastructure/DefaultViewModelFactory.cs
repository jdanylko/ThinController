...
namespace ThinController.Infrastructure
{
    public class DefaultViewModelFactory : IViewModelFactory
    {
        public TViewModel GetViewModel<TController, TViewModel>(TController controller)
        {
            ...
            var modelBuilder = container.GetInstance<IViewModelBuilder<TController, 
                TViewModel>>();
            ...
            if (modelBuilder == null)
                throw new Exception(
                    string.Format(
                        "Could not find a ModelBuilder with a {0} Controller/{1} " +
        ...
        public TViewModel GetViewModel<TController, TViewModel, TInput>(TController controller, 
            TInput data)
        {
            var container = (controller as BaseController).Container;
            ...
            if (modelBuilder == null)
                throw new Exception(
                    string.Format(
                        "Could not find a ModelBuilder with a {0} Controller/{1} ViewModel/{2} " +
                        "TInput pairing. Please create one.",
